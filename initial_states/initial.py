...
def qaoa(gb, *a):
    ...
    state = random_k_state(G, len(G.nodes), k)
...
def classical(gi, p, s):
    ...
    num_trials = 20
    for i in range(s):
        sample_g, sample_b = common.LHS(p, num_trials, 0, pi/2, 0, pi)
        init = [[0,pi/2] if i < p else [0,pi] for i in range(2*p)]
        exp = -1
        for i in range(num_trials):
            kwargs = {'method': 'L-BFGS-B', 'args': (G, C, M, k, p), 'bounds': init}
            optimal = basinhopping(qaoa, [sample_g[i], sample_b[i]], minimizer_kwargs=kwargs, niter=2, disp=True)
            if -optimal.fun > exp:
                exp = -optimal.fun
        print('exp: ' + str(exp))
    return data
